"""End-to-end crossed upload probes; source texts repeat the synthetic holdout."""
...
MANIFEST = json.loads((ROOT / "file_probe_crossed_manifest.json").read_text(encoding="utf-8"))
GOLD = {row["case_id"]: row for row in map(json.loads, (ROOT / "holdout_v2_gold.jsonl").read_text(encoding="utf-8").splitlines())}
OUT = ROOT / "file_probe_crossed_predictions.jsonl"
...
with httpx.Client(timeout=40.0) as client, OUT.open("w", encoding="utf-8") as output:
    for item in MANIFEST:
        ...
        output.flush()
        print(row["case_id"], item["language"], item["context_id"], item["format"], row["gold"], result.get("decision", result.get("error_code")), result["latency_ms"], flush=True)
        time.sleep(max(0, 2.2 - result["latency_ms"] / 1000))

(ROOT / "file_probe_crossed_run_manifest.json").write_text(json.dumps({
    "started_at_utc": started_at,
    "completed_at_utc": datetime.now(timezone.utc).isoformat(),
    "n_probes": len(MANIFEST),
    "purpose": "crossed_file_extraction_and_transport_regression_not_independent_quality_validation",
}, indent=2) + "\n", encoding="utf-8")
