"""One pass over the frozen multilingual synthetic set."""
...
GOLD = ROOT / "multilingual_gold.jsonl"
EXPECTED = (ROOT / "multilingual_gold.sha256").read_text(encoding="utf-8").split()[0]
assert hashlib.sha256(GOLD.read_bytes()).hexdigest() == EXPECTED
OUT = ROOT / "multilingual_predictions_first_pass.jsonl"
assert not OUT.exists(), "Refuse to overwrite first-pass predictions"
...
(ROOT / "multilingual_run_manifest.json").write_text(json.dumps({
    "gold_sha256": EXPECTED,
    "started_at_utc": started_at,
    "completed_at_utc": datetime.now(timezone.utc).isoformat(),
    "n_cases": len(cases),
    "source_type": "self_authored_synthetic",
    "purpose": "independent_multilingual_context_diagnostic",
    "labels_frozen_before_first_provider_call": True,
}, indent=2) + "\n", encoding="utf-8")
